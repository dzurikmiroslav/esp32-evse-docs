...
Socket-outlet mode requires a Proximity Pilot to be present on the board; the firmware refuses to enable socket-outlet mode without it. With a fixed cable, PP is not needed and the maximum current is governed only by the configured maximum charging current.
...
The three `levels` are the measured ADC voltages (in mV) at the 13&nbsp;A, 20&nbsp;A and 32&nbsp;A boundaries; readings below the 32&nbsp;A threshold are classified as 63&nbsp;A. As with the control pilot, the exact figures depend on the board's divider and should be measured for your design.
